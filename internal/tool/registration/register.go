...
var Register = bitrise.Tool{
	...
	Definition: mcp.NewTool("register",
		mcp.WithDescription("Start registration for a new Bitrise user. Sends a one-time password (OTP) to the provided email address. Returns a `pending_signup_id`. After this returns successfully, ask the user for the OTP that was sent to their email, then call `verify_registration` with that OTP and the `pending_signup_id` from this response."),
...
// apiErrorResult converts an *bitrise.APIError into a structured tool result so
