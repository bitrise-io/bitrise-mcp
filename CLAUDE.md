...
- One Bitrise account covers both product areas: identity and workspace lookup are the shared `me` and `list_workspaces` tools (also in the `dev-environments` groups). The Dev Environments user UUID is never surfaced as a user identity (no field on `me`, no lookup tool); it stays only as the opaque `ownerId` on session and warm-pool records, exactly as the Dev Environments API returns it, because listings are scoped to the caller and warm pools carry `createdByEmail`. Do not add product-specific identity tools or fields.
